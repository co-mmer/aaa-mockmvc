...
class TestArrangeGetUrlImplTest {
  ...
  void setUp() {
    ...
    this.impl = new TestArrangeGetUrlImpl(this.context, TEST_URL);
  }
  ...
  class constructor {
    ...
    void GIVEN_url_WHEN_arrangeUrl_THEN_setUriIsCalled() {
      ...
      mockTestArrangeUrlUtils.verify(
          () -> setUri(context.getArrangeResult().getUrl(), GET, TEST_URL));
    }

    @Test
    void GIVEN_pathWithVariable_WHEN_call_constructor_THEN_setUriIsCalled() {
      // Act
      new TestArrangeGetUrlImpl(context, TEST_PATH_VAR1, VAR_STRING_1);

      // Assert
      mockTestArrangeUrlUtils.verify(
          () -> setUri(context.getArrangeResult().getUrl(), GET, TEST_PATH_VAR1, VAR_STRING_1));
    }
    ...
    void GIVEN_uri_WHEN_call_constructor_THEN_setUriIsCalled() {
      // Act
      new TestArrangeGetUrlImpl(context, TEST_URI);

      // Assert
      mockTestArrangeUrlUtils.verify(
          () -> setUri(context.getArrangeResult().getUrl(), GET, TEST_URI));
    }
  }
