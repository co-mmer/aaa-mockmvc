 ...
 *
 * <p><b>Scope:</b> Provides a fluent API to define request headers ({@code Accept},
 * {@code Authorization}, and arbitrary custom headers) for a pending DELETE request. Header names
 * are treated case-insensitively, and multi-value headers are supported. These steps mutate only
 * the request specification; no network I/O is performed until {@code act().perform()} is invoked.
 *
